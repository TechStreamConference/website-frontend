...
<dialog open={_show} on:click={deny} role="presentation" transition:fade={{ duration: 300 }}>
	<div class="general-popup-modal" on:click={(e) => e.stopPropagation()} role="presentation">
		<SubHeadline classes="white">{headline}</SubHeadline>
		<Paragraph>{text}</Paragraph>
		<div class="general-popup-button-wrapper">
			<Button on:click={deny} ariaLabel="Klicke hier, um die Abfrage zu verneinen"
				>{denyButtonText}</Button
			>
			<Button on:click={accept} ariaLabel="Klicke hier, um die Abfrage zu aktzeptern"
