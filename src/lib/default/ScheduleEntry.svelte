<script lang="ts">
    ...
    import ScheduleTag from '@/lib/default/ScheduleTag.svelte';
    import ScheduleSpeaker from '@/lib/default/ScheduleSpeaker.svelte';
...
<div class="schedule-entry-horizontal-line">
    <SubHeadline classes="schedule-entry-time">{format_date(talk.starts_at, '%hh:%mm Uhr')}</SubHeadline>
</div>
<div class="schedule-entry-entry-wrapper schedule-entry-horizontal-line">
    ...
    <div class="schedule-entry-speaker-wrapper">
        <ScheduleSpeaker
                {speaker}
                on:click={(event) => {
			person_popup_callback(event, speaker);
		}}
        />
        {#if talk.guests.length > 0}
            ...
            {#each talk.guests as guest}
                <ScheduleSpeaker
                        speaker={guest}
                        on:click={(event) => {person_popup_callback(event, guest);}}
                />
            {/each}
        {/if}
    </div>
