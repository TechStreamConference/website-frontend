...
export function validateData(data: SetAdminEvent, allSpeaker: SetAllAdminEventSpeaker): string[] {
    ...
    if (data.publish_date
        && data.schedule_visible_from
        && !isBeforeOrSameDatesString(data.publish_date, data.schedule_visible_from)) {

        errorList.push('Der Ablaufplan ist vor dem Event sichtbar.');
    }

    if (data.publish_date
        && !isBeforeOrSameDatesString(data.publish_date, data.start_date)) {

        errorList.push('Das Event wird erst nach dem Event-Start veröffentlicht.');
    }

    if (data.schedule_visible_from
        && !isBeforeOrSameDatesString(data.schedule_visible_from, data.start_date)) {

        errorList.push('Der Ablaufplan ist erst nach dem Event-Start sichtbar.');
    }

    if (data.call_for_papers_start
        && data.call_for_papers_end
        && !isBeforeOrSameDatesString(data.call_for_papers_start, data.call_for_papers_end)) {
...
// speaker
    for (const speaker of allSpeaker) {
        if (speaker.visible_from
            && !isBeforeOrSameDatesString(speaker.visible_from, data.start_date)) {
